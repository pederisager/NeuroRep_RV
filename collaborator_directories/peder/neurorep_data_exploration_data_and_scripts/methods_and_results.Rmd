...
```{r, echo=FALSE}
kable(jou.freq, caption = "Journals most frequently published in.", row.names = FALSE)
```
...
![VOSviewer map of title/abstract keyword co-occurences](vosviewer_label_cooccurence.png)
...
```{r echo=FALSE}
metric.names <- c("WoS", "Crossref", "Scopus", "CWTS", "CWTS normalized", "Altmetric", "Total")
metric.desc <- c(codebook[81, 2], codebook[78, 2], codebook[79, 2], codebook[89, 2], codebook[91, 2], codebook[80, 2], "Number of articles for which all citation metrics were available")
metric.n <- c(sum(!is.na(data.bib$TC_2020)),
              ...
              sum(complete.cases(data.bib[, c("TC_2020", "scopus_citations", "crossref_citations", "altmetric_score", "tcs", "tncs")])))
...
# Number of articles for which all citation data exists
sum(complete.cases(data.bib[, c("TC_2020", "scopus_citations", "crossref_citations", "altmetric_score", "tcs", "tncs")]))
...
RValt.p <- ggplot(data = data.all, aes(x = RV_alt)) +
  ...
  theme_classic() +
  labs(y="", x=expression(RV[alt]~density)) + 
  theme(
...
RVwos.p <- ggplot(data = data.all, aes(x = RV)) +
  ...
  theme_classic() +
  labs(y="", x=expression(RV[WoS]~density)) + 
  theme(
  axis.text.x = element_blank(),
  axis.text.y = element_blank(),
  axis.ticks = element_blank()) 
...
RVcor.p <- ggplot(data = data.all, aes(x = RV, y = RV_alt)) +
  ...
  #geom_rug(data=data.all, mapping=aes(x=RV), color="brown1", sides="b") +
  #geom_rug(data=data.all, mapping=aes(y=RV_alt), color="cornflowerblue", sides="l") +
  geom_point() +
...
# gridExtra::grid.arrange(RVwos.p, RValt.p, RVcor.p, layout_matrix = rbind(c(2, 3, 3), c(1, 3, 3)))
# 
...
# 
# 
# grid.arrange(RValt.p, RVcor.p, empty, RVwos.p, ncol=2, nrow=2, widths=c(1, 4), heights=c(4, 1))
plot(RVcor.p)
```
