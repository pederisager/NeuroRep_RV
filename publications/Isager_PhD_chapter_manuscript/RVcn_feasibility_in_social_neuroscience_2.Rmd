...
#setwd("publications/Isager_PhD_chapter_manuscript/OSF_files/analysis/")

# source("load_and_clean_data.R")  # Can be run to generate datasets from raw data files

codebook <- read.csv("publications/Isager_PhD_chapter_manuscript/OSF_files/raw_data/codebook.csv")  # Codebook with variable descriptions of all variables in data.bib, data.all, and data.irr
data.bib <- readRDS("publications/Isager_PhD_chapter_manuscript/OSF_files/processed_data/data_bib.Rds")  # Bibliometric data for all articles that survived keyword exclusion
data.all <- readRDS("publications/Isager_PhD_chapter_manuscript/OSF_files/processed_data/data_all.Rds")  # Bibliometric data + sample size of all individual studies selected for sample size coding
data.irr <- readRDS("publications/Isager_PhD_chapter_manuscript/OSF_files/processed_data/data_irr.Rds")  # Sample size inter-rater reliability data for sample of studies selected for triple-coding

# Add total citation count from Scite data to data.bib
data.scite <- read.csv("publications/Isager_PhD_chapter_manuscript/OSF_files/processed_data/data_scite.csv")  # Dataset containing total Scite citation count
...
RVcor.p <- ggplot(data = data.all, aes(x = RV, y = RV_alt)) +
  geom_rug() +
  geom_rug(data = highestRV, color="brown1", sides="b", linewidth=1) +
...
# Load raw data
data.survey <- read.csv("publications/Isager_PhD_chapter_manuscript/OSF_files/raw_data/SM2_survey_data.csv", header = T)
imp.dat <- data.survey[, grep("_imp", names(data.survey))]  ## Ratings
rank.dat <- data.survey[, grep("_rank", names(data.survey))]  ## Ranks
# Load summary table
data.survey.summary <- read.csv("publications/Isager_PhD_chapter_manuscript/OSF_files/processed_data/SM2_summary_table.csv", header = T)
