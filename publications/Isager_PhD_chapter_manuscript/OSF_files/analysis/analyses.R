...
setwd("C:/Users/pedisa94/Dropbox/jobb/Projects/2019_NeuroRep_Replication_Value/publications/Isager_PhD_chapter_manuscript/OSF_files/analysis/")
...
# Load raw data
data.survey <- read.csv("../raw_data/SM2_survey_data.csv", header = T)
imp.dat <- data.survey[, grep("_imp", names(data.survey))]  ## Ratings
rank.dat <- data.survey[, grep("_rank", names(data.survey))]  ## Ranks
# Load summary table
data.survey.summary <- read.csv("../processed_data/SM2_summary_table.csv", header = T)
